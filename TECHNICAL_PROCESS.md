...
| 5f Départ | `scripts/depart.py` | `public/partir.html` — the departure instrument: the most restrictive declared obstructions, the reader's corridors marked on-device (streets island + literal folding; no account, no position), what changed since the last edition, the seal. One small inline script is the only enhancement; the page is complete without it. `scripts/promesse.py` feeds the official-presence line ("Aucun document officiel dans les N éditions suivies") on the brief's dossiers, the récit pages and the departure screen — counted only over editions whose record actually carried the official counter, never a verdict |
