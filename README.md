...
**Front-end law (v0.2).** The brief is an answer-first instrument: « En un coup d'œil » opens the edition with measured facts and jump links to the sections; a sticky masthead, scroll-spy and a Ctrl/⌘-K command palette make finding effortless; continuity (the newest articles since your marker) stays on-device. Type is self-hosted variable Newsreader/Figtree — no external font request on any surface — with `prefers-color-scheme` and `prefers-contrast` support. `DESIGN.md` is the law. Each dossier also shows a voice roster (every followed institution, spoke or quiet) and a collapsed collection timeline, including field-level **question revisions** quoted verbatim; the roadworks section carries a static, self-hosted spatial scheme (dot density with legend and scale — a distribution reading, never a map or geographic proof) and opt-in saved corridors (declared street names, literal match, on-device only, no geolocation, no computed route effect). The brief is mobile-first — finger-sized targets, safe-area aware, no focus zoom — and installable via `site.webmanifest` (no service worker, no tracking). Preview images are content-addressed and cached immutably, with intrinsic dimensions measured from the file header; there is no generated `srcset` (the feeds offer no smaller variants and the pipeline is stdlib-only).
